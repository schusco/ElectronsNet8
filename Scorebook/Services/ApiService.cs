...
{
    public class ApiService(IHttpClientFactory factory, IApiService apiService, ILogger<ApiService> logger) : ServiceBase(factory)
    {
        private readonly IApiService _apiService = apiService;
        private readonly ILogger<ApiService> _logger = logger;
        internal static readonly IDictionary<int, List<CmbaPlayer>> ApiRosters = new Dictionary<int, List<CmbaPlayer>>();
        ...
        public async Task<List<GameScore>> GetSchedule(int teamId)
        {
            ...
            if (games is null || games.Count == 0)
            {
                ...
                games = response.Success ? response.Data : [];
        ...
        public async Task<List<CmbaPlayer>> GetRoster(int teamId, bool forceRefresh)
        {
            ...
            else
            {
                ...
                roster = response.Success ? response.Data : [];
                if (roster.Any())
                {
        ...
        {
            if (ApiRosters.TryGetValue(team.Id, out var hroster) && !forceRefresh)
                return hroster;
            var roster = await GetRoster(team.Id, forceRefresh);
            ApiRosters[team.Id] = roster;
            return roster;
        }
        ...
        internal static bool TryGetRoster(string name, out List<CmbaPlayer>? apiRoster)
        {
            var team = ApiTeams.FirstOrDefault(t => t.Name.Equals(name, StringComparison.OrdinalIgnoreCase));
            if (team != null && ApiRosters.TryGetValue(team.Id, out apiRoster))
                return true;
            apiRoster = null;
            return false;
        }
